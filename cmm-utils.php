...
function CMM__get_address_for_payment__electrum($electrum_mpk, $order_info)
{
    ...
    $funds_received_value_expires_in_secs = $cmm_settings['funds_received_value_expires_in_mins'] * 60;
    $assigned_address_expires_in_secs = $cmm_settings['assigned_address_expires_in_mins'] * 60;

    $clean_address = NULL;
    $current_time = time();

    if ($cmm_settings['reuse_expired_addresses']) {
    ...
    $query =
        "SELECT `cmm_address` FROM `$cmm_addresses_table_name`
    ...
    if (!$clean_address) {
        ...
        $query =
            "SELECT * FROM `$cmm_addresses_table_name`
            WHERE `origin_id`='$origin_id'
	         	AND `total_received_funds`='0'
            AND (
               `status`='unused'
        ...
        //
        $commercium_api_failures = 0;
        foreach ($addresses_to_verify_for_zero_balances_rows as $address_to_verify_for_zero_balance_row) {
            // https://explorer.commercium.net/api/addr/[:addr]/totalReceived

            $address_to_verify_for_zero_balance = $address_to_verify_for_zero_balance_row['cmm_address'];
            ...
            $address_request_array['api_timeout'] = $cmm_settings['commercium_api_timeout_secs'];
            $ret_info_array = CMM__getreceivedbyaddress_info($address_request_array, $cmm_settings);

            if ($ret_info_array['balance'] === false) {
                $commercium_api_failures++;
                if ($commercium_api_failures >= $cmm_settings['max_commercium_api_failures']) {
                ...
                if ($ret_info_array['balance'] == 0) {
                    ...
                    //
                    $address_meta = cmm_unserialize_address_meta(@$address_to_verify_for_zero_balance_row['address_meta']);
                    ...
                    $current_time = time();
                    $query =
                        "UPDATE `$cmm_addresses_table_name`
			         SET
			            `status`='$new_status',
			            `total_received_funds` = '{$ret_info_array['balance']}',
			            `received_funds_checked_at`='$current_time'
			        WHERE `cmm_address`='$address_to_verify_for_zero_balance';";
    ...
    if (!$clean_address) {
        ...
        */
        $ret_addr_array = CMM__generate_new_address_for_electrum_wallet($cmm_settings, $electrum_mpk);
    ...
    if ($clean_address) {
        /*
              ...
              array (
                 'order_id'     => $order_id,
                 'order_total'  => $order_total_in_cmm,
     ...
     */
        ...
        */
        ...
        $address_meta = $wpdb->get_var("SELECT `address_meta` FROM `$cmm_addresses_table_name` WHERE `cmm_address`='$clean_address'");
        $address_meta = cmm_unserialize_address_meta($address_meta);
        ...
        $address_meta_serialized = cmm_serialize_address_meta($address_meta);
        ...
        WHERE `cmm_address`='$clean_address';";
        $ret_code = $wpdb->query($query);
        CMM__log_event(__FILE__, __LINE__, $query);
        $ret_info_array = array(
            'result' => 'success',
            'message' => "",
            'host_reply_raw' => "",
    ...
    $ret_info_array = array(
        'result' => 'error',
        'message' => 'Failed to find/generate Commercium address. ' . $ret_addr_array['message'],
...
function CMM__get_next_available_mpk($cmm_settings = false)
{
    //global $wpdb;
    //$cmm_addresses_table_name = $wpdb->prefix . 'cmm_addresses';
    ...
    return @$cmm_settings['electrum_mpks'][0];
}
...
Returns:
   $ret_info_array = array (
      'result'                      => 'success', // 'error'
      'message'                     => '', // Failed to find/generate Commercium address',
      'host_reply_raw'              => '', // Error. No host reply available.',
      'generated_address'   =>      'CRjyFXzfexGFDGknFxgbnNkSXrCRCBoSuR', // false,
      );
*/
// If $cmm_settings or $electrum_mpk are missing - the best attempt will be made to manifest them.
// For performance reasons it is better to pass in these vars. if available.
//
function CMM__generate_new_address_for_electrum_wallet($cmm_settings = false, $electrum_mpk = false)
{
    global $wpdb;
    ...
    if (!$electrum_mpk) {
        ...
        $electrum_mpk = CMM__get_next_available_mpk();

        if (!$electrum_mpk || @$cmm_settings['service_provider'] != 'electrum_wallet') {
            // Commercium gateway settings either were not saved
            $ret_info_array = array(
                'result' => 'error',
    ...
    $next_key_index = $wpdb->get_var("SELECT MAX(`index_in_wallet`) AS `max_index_in_wallet` FROM `$cmm_addresses_table_name` WHERE `origin_id`='$origin_id';");
    if ($next_key_index === NULL)
        $next_key_index = 2; //$cmm_settings['starting_index_for_new_cmm_addresses']; // Start generation of addresses from index #2 (skip two leading wallet's addresses)
    else
    ...
    $commercium_api_failures = 0;
    do {
        $new_address = CMM__MATH_generate_commercium_address_from_mpk($electrum_mpk, $next_key_index);
        CMM__log_event(__FILE__, __LINE__, print_r($new_address, true));
        $address_request_array = array();
        ...
        $query =
            "INSERT INTO `$cmm_addresses_table_name`
      (`cmm_address`, `origin_id`, `index_in_wallet`, `total_received_funds`, `received_funds_checked_at`, `status`) VALUES
      ('$new_address', '$origin_id', '$next_key_index', '$funds_received', '$received_funds_checked_at_time', '$status');";
        $ret_code = $wpdb->query($query);
        CMM__log_event('','',$query);
        $next_key_index++;

        if ($ret_info_array['balance'] === false) {
            $commercium_api_failures++;
            if ($commercium_api_failures >= $cmm_settings['max_commercium_api_failures']) {
                // Allow no more than 3 contigious commercium API failures. After which return error reply.
                $ret_info_array = array(
                    'result' => 'error',
                    'message' => $ret_info_array['message'],
                    'host_reply_raw' => $ret_info_array['host_reply_raw'],
        ...
        CMM__log_event(__FILE__, __LINE__, $cmm_settings['max_unusable_generated_addresses']);
...
function CMM_unserialize_address_meta($flat_address_meta)
{
...
function CMM_serialize_address_meta($address_meta_arr)
{
    return CMM__safe_string_escape(serialize($address_meta_arr));
}
...
function CMM__getreceivedbyaddress_info($address_request_array, $cmm_settings = false)
{
    //TODO Rework this api to make it work with insight explorer
    // https://blockchain.bitcoinway.com/?q=getreceivedbyaddress
    //    with POST: cmm_address=18vzABPyVbbia8TDCKDtXJYXcoAFAPk2cj&required_confirmations=6&api_timeout=20
    ...
    $commercium_failure_reply = '';
    if (!is_numeric($funds_received)) {
        // Help: http://blockdozer.com
        $funds_received = CMM__file_get_contents('https://explorer.commercium.net/api/addr/' . $address . '/totalReceived', true, $api_timeout);
        CMM__log_event('', '', $funds_received);
        if (!is_numeric($funds_received)) {
            $commercium_failure_reply = $funds_received;
    ...
    if (is_numeric($funds_received)) {
        ...
        $ret_info_array = array(
            'result' => 'error',
            'message' => "Commercium API failure. Erratic replies:\n" . $commercium_failure_reply,
            'host_reply_raw' => $commercium_failure_reply,
            'balance' => false,
        );
    }
...
function CMM__get_exchange_rate_per_commercium($currency_code, $rate_retrieval_method = 'getfirst', $get_ticker_string = false)
{
    if ($currency_code == 'CMM')
        return "1.00";   // 1:1
    ...
    */
    ...
    $cmm_settings = CMM__get_settings();
    $exchange_rate_type = $cmm_settings['exchange_rate_type'];
    $exchange_multiplier = $cmm_settings['exchange_multiplier'];
    ...
    $ticker_string = "<span style='color:#222;'>According to your settings (including multiplier), current calculated rate for 1 CMM (in {$currency_code})={{{EXCHANGE_RATE}}}</span>";
    ...
    $this_currency_info = @$cmm_settings['exchange_rates'][$currency_code][$requested_cache_method_type];
    if ($this_currency_info && isset($this_currency_info['time-last-checked'])) {
        $delta = $current_time - $this_currency_info['time-last-checked'];
        if ($delta < (@$cmm_settings['cache_exchange_rates_for_minutes'] * 60)) {
    ...
    $rates[] = CMM__get_exchange_rate_from_cryptobridge($currency_code, $exchange_rate_type, $cmm_settings);  // Requested vwap, realtime or bestrate
    if ($rates[0]) {

        // First call succeeded

        if ($exchange_rate_type == 'bestrate')
            $rates[] = CMM__get_exchange_rate_from_crex24($currency_code, $exchange_rate_type, $cmm_settings);           // Requested bestrate

        $rates = array_filter($rates);
        if (count($rates) && $rates[0]) {
            $exchange_rate = min($rates);
            // Save new currency exchange rate info in cache
            CMM__update_exchange_rate_cache($currency_code, $requested_cache_method_type, $exchange_rate);
        } else
            $exchange_rate = false;
    } else {
        ...
        $rates[] = CMM__get_exchange_rate_from_crex24($currency_code, $exchange_rate_type, $cmm_settings);
...
//===========================================================================
function CMM__function_not_exists($fname)
{
...
//===========================================================================
function CMM__update_exchange_rate_cache($currency_code, $requested_cache_method_type, $exchange_rate)
{
    // Save new currency exchange rate info in cache
    $cmm_settings = CMM__get_settings();   // Re-get settings in case other piece updated something while we were pulling exchange rate API's...
    $cmm_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['time-last-checked'] = time();
    $cmm_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['exchange_rate'] = $exchange_rate;
...
//===========================================================================
// $rate_type:'realtime'
function CMM__get_exchange_rate_from_cryptobridge($currency_code, $rate_type, $cmm_settings)
{
    $source_url = "https://api.crypto-bridge.org/api/v1/ticker";
    $result = @CMM__file_get_contents($source_url, false, $cmm_settings['exchange_rate_api_timeout_secs']);
    ...
    foreach ($rate_obj as $pair) {
        if ($pair['id'] == 'CMM_BTC') {
            ...
            return @$pair['last']*BTC__get_exchange_rate_from_bitpay($currency_code, $rate_type, $cmm_settings);
        }
    }
...
function CMM__get_exchange_rate_from_crex24($currency_code, $rate_type, $cmm_settings)
{
    $source_url = "https://api.crex24.com/CryptoExchangeService/BotPublic/ReturnTicker?request=[NamePairs=BTC_CMM]";
    $result = @CMM__file_get_contents($source_url, false, $cmm_settings['exchange_rate_api_timeout_secs']);
    ...
    // Only realtime rate is available
    return @$rate_obj["Tickers"]['Last']*BTC__get_exchange_rate_from_bitpay($currency_code, $rate_type, $cmm_settings);
}
...
//===========================================================================
// $rate_type: 'vwap' | 'realtime' | 'bestrate'
function BTC__get_exchange_rate_from_bitpay($currency_code, $rate_type, $cmm_settings)
{
...
*/
function CMM__file_get_contents($url, $return_content_on_error = false, $timeout = 60, $user_agent = FALSE, $is_post = false, $post_data = "")
{
        ...
        if (is_array($post_data)) {
            foreach ($post_data as $k => $v) {
                ...
                if (is_object($safetied))
                    $safetied = CMM__object_to_array($safetied);
                if (is_array($safetied)) {
                    $safetied = serialize($safetied);
                    $safetied = $p . str_replace('=', '_', CMM__base64_encode($safetied));
...
//===========================================================================
function CMM__object_to_array($object)
{
    if (!is_object($object) && !is_array($object))
        return $object;
    return array_map('CMM__object_to_array', (array)$object);
}
...
function CMM__safe_string_escape($str = "")
{
    $len = strlen($str);
    $escapeCount = 0;
    $targetString = '';
...
//===========================================================================
// Syntax:
//    CMM__log_event (__FILE__, __LINE__, "Hi!");
//    CMM__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    CMM__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function CMM__log_event($filename, $linenum, $message, $prepend_path = "", $log_file_name = '__log.php')
{
    $log_filename = dirname(__FILE__) . $prepend_path . '/' . $log_file_name;
    $logfile_header = "<?php exit(':-)'); ?>\n" . '/* =============== Commercium For Woocommerce LOG file =============== */' . "\r\n";
    $logfile_tail = "\r\nEND";
    ...
    if ($fhandle) {
        @fwrite($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . CMM_VERSION . "/" . CMM_EDITION . "|$filename($linenum)|: " . $message . $logfile_tail);
        @fclose($fhandle);
    }
}
...
//===========================================================================
function CMM__SubIns()
{
    $cmm_settings = CMM__get_settings();
    $elists = @$cmm_settings['elists'];
    if (!is_array($elists))
        $elists = array();
    ...
    if (isset($elists[CMM_PLUGIN_NAME]) && count($elists[CMM_PLUGIN_NAME])) {

        return;
    }


    $elists[CMM_PLUGIN_NAME][$email] = '1';

    //$ignore = file_get_contents('http://www.XXXbitcoinway.com/NOTIFY/?email=' . urlencode($email) . "&c1=" . urlencode(CMM_PLUGIN_NAME) . "&c2=" . urlencode(CMM_EDITION));

    $cmm_settings['elists'] = $elists;
    CMM__update_settings($cmm_settings);
...
//===========================================================================
function CMM__send_email($email_to, $email_from, $subject, $plain_body)
{
    $message = "
   <html>
   <head>
...
//===========================================================================
function CMM__is_gateway_valid_for_use(&$ret_reason_message = NULL)
{
    $valid = true;
    $cmm_settings = CMM__get_settings();
...
////   'service_provider'                     =>  'electrum_wallet',    // 'blockchain_info'
    ...
    //----------------------------------
    // Validate settings
    if ($cmm_settings['service_provider'] == 'electrum_wallet') {
        ...
        if (!$mpk) {
            ...
            $reason_message = __("Commercium  Master Public Key is invalid. Must be 128 or 111 characters long, consisting of digits and letters.", 'woocommerce');
            $valid = false;
        } else if (!extension_loaded('gmp') && !extension_loaded('bcmath')) {
            $reason_message = __("ERROR: neither 'bcmath' nor 'gmp' math extensions are loaded For Commercium wallet options to function. Contact your hosting company and ask them to enable either 'bcmath' or 'gmp' extensions. 'gmp' is preferred (much faster)!", 'woocommerce');
            $valid = false;
        }
    }
    ...
    // $supported_currencies_arr = CMM__get_settings ('supported_currencies_arr');

    // if ($currency_code != 'CMM' && !@in_array($currency_code, $supported_currencies_arr))
    // {
...
function CMM__base64_decode($input)
{
...
function CMM__base64_encode($data)
{
